...
The frontend uses Next.js 14 with React Query and TailwindCSS. TypeScript types exactly mirror backend Pydantic models.
...
|----------|-------|----------|
| File-Based | 14 | CSV, Parquet, Excel, EBCDIC, Fixed-Width |
| Database | 9 | Postgres, MySQL, Snowflake, Oracle, DB2 |
| Streaming | 8 | Kafka, Pub/Sub, Kinesis, EventHub |
| API & SaaS | 12 | REST, GraphQL, Salesforce, SAP |
| Legacy | 7 | DTSX, AS400, Mainframe, COBOL |
| NoSQL | 9 | MongoDB, Cassandra, DynamoDB |
| Logs | 5 | Splunk, Datadog, CloudWatch |
| Cloud | 4 | S3, GCS, Azure Blob |
| Advanced | 6 | CDC, Delta Lake, Iceberg |

#### 3 Input Modes
1. **UI Structured** - Type-safe form with category-based source selection
2. **Natural Language** - LLM converts NL → structured metadata (NEVER executes directly)
3. **DTSX Migration** - Parses SSIS packages and maps to Airflow

#### Critical Frontend Rules
| DO ✅ | NEVER ❌ |
|-------|----------|
| Mirror Pydantic types in TypeScript | Create divergent frontend types |
| Use canonical types (`pipeline-canonical.ts`) | Use legacy types |
| Convert NL to structured metadata | Execute natural language directly |
| Type-specific source config forms | Generic forms |
| React Query for API state | Manual fetch with useState |

#### Key Files
- `pipeline-canonical.ts` → 680 lines, all canonical types
- `UnifiedPipelineForm.tsx` → Main form orchestrator
- `SourceTypeSelector.tsx` → Category-based source picker
- `api.ts` → 7 new canonical API methods
