...
async def approve_langgraph_workflow(
    ...
    # Get extracted parameters from workflow state (set by LLM during classify/plan)
    extracted_params = workflow_state.get("extracted_params", {})

    # Extract instance_name, zone, project from LLM-extracted params or incident description
    import re
    ...
    # Prioritize LLM-extracted params, fallback to regex, then defaults
    script_args = {
        "instance_name": extracted_params.get("instance_name") or (instance_match.group(1) if instance_match else "test-incident-vm-01"),
        "zone": extracted_params.get("zone") or (zone_match.group(1) if zone_match else "us-central1-a"),
        "project": extracted_params.get("project") or (project_match.group(1) if project_match else "agent-ai-test-461120"),
    }

    # Log extracted parameters for debugging
    logger.info(f"Execution parameters: instance={script_args['instance_name']}, zone={script_args['zone']}, project={script_args['project']}")
...
async def retry_github_trigger(incident_id: str):
    ...
    if not inputs:
        ...
        # Get extracted parameters from workflow state (set by LLM)
        extracted_params = workflow_state.get("extracted_params", {})
