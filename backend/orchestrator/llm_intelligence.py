...
class LLMIntelligence:
    ...
    async def classify_incident(
        ...
        """
        Classify incident and extract required parameters.
        ...
        Returns:
            Classification with category, confidence, and extracted parameters
        """
        # Build incident object for analysis
        incident = {
            "incident_id": metadata.get("sys_id", "unknown"),
            "short_description": description,
            "description": description,
            "category": metadata.get("category", ""),
            "priority": metadata.get("severity", "3"),
            "service": service
        }

        # Analyze incident to get classification and parameters
        analysis = analyze_incident_with_llm(incident)

        # Return classification format expected by workflow
        return {
            "category": analysis.get("service_type", "infrastructure"),
    ...
    async def generate_remediation_plan(
        ...
        """
        Generate remediation plan with extracted parameters.
        ...
        # Build incident object for parameter extraction
        incident = {
            "incident_id": incident_context.get("sys_id", "unknown"),
            "short_description": incident_context.get("description", ""),
        ...
        # Extract parameters based on script requirements
        script_type = top_result.get("metadata", {}).get("type", "shell")
        required_params = []

        if "gcp" in classification.lower() or "vm" in incident["description"].lower():
            required_params = ["instance_name", "zone", "project"]
        elif "kubernetes" in classification.lower() or "k8s" in classification.lower():
            required_params = ["namespace", "pod_name", "deployment_name"]

        # Extract parameters using LLM
        extracted_params = {}
        if required_params:
            extracted_params = extract_parameters_with_llm(incident, required_params)
        ...
        return {
            ...
            "steps": [
                ...
                {"step": 2, "action": f"Execute {top_result.get('id', 'script')}", "timeout": 300},
                {"step": 3, "action": "Verify system health post-execution", "timeout": 60}
            ],
            "reasoning": f"Based on incident classification '{classification}', selected script '{top_result.get('id')}' with {top_result.get('score', 0)*100:.1f}% confidence",
            "estimated_duration": 120,
            "risk_assessment": top_result.get("metadata", {}).get("risk_level", "medium"),
