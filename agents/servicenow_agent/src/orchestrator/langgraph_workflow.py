...
try:
    from agents.servicenow_agent.src.rag.swarm_retriever import SwarmRetriever
except ImportError:
    SwarmRetriever = None
from agents.servicenow_agent.src.orchestrator.llm_judge import LLMJudge
from agents.servicenow_agent.src.orchestrator.llm_intelligence import LLMIntelligence
from agents.servicenow_agent.src.utils.github_actions import GitHubActionsClient
from agents.servicenow_agent.src.utils.slack_notifier import SlackNotifier
...
async def node_feedback_loop(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        # 3. Index resolution into Weaviate for future semantic search
        try:
            from agents.servicenow_agent.src.rag.rag_updater import get_rag_updater
            updater = get_rag_updater()
            await updater.index_resolution(
                incident_id=incident_id,
                description=state.get("short_description", state.get("description", "")),
                resolution_script=script_id or "",
                resolution_notes=state.get("resolution_summary", ""),
                outcome="success" if state.get("fix_verified") else "failed",
                category=state.get("classification", ""),
                service=service,
            )
        except Exception as rag_err:
            logger.warning("rag_updater_failed", error=str(rag_err))
