...
class ScriptSelector:
    ...
    def __init__(self, registry_path: str = "/app/agents/servicenow_agent/data/registry.json"):
        self.registry_path = registry_path
        self.registry = self._load_registry()
        self.scripts = self.registry.get("scripts", [])
        logger.info(f"ScriptSelector initialized with {len(self.scripts)} scripts from {registry_path}")
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    from agents.servicenow_agent.src.config.settings import get_settings
    from agents.servicenow_agent.src.control_plane import (
        get_control_plane,
        WorkflowType,
    )
    from agents.servicenow_agent.src.control_plane.handlers import ITServiceHandler, DataPipelineHandler, get_data_agent_handler
    from agents.servicenow_agent.src.streaming.event_publisher import get_publisher
    ...
    try:
        from agents.servicenow_agent.src.utils.otel_tracing import setup_tracing
        otel_initialized = setup_tracing(
            ...
            enable_fastapi=True, enable_redis=True, enable_kafka=True, enable_httpx=True,
        )
        if otel_initialized:
    ...
    langfuse_ok = False
    try:
        from agents.servicenow_agent.src.orchestrator.llm_intelligence import langfuse_client, LANGFUSE_ENABLED
    ...
    try:
        from agents.servicenow_agent.src.orchestrator.metrics import SYSTEM_INFO
        logger.info("Prometheus metrics registered (60+ metrics at /metrics)")
    ...
    # Start EventOrchestrator Kafka consumer as background task
    ...
    try:
        ...
        logger.info("EventOrchestrator Kafka consumer started as background task")
    except Exception as e:
        logger.warning(f"Failed to start EventOrchestrator: {e} - workflows must be triggered manually")
    ...
    if event_orchestrator_task:
        try:
            app.state.event_orchestrator.running = False
    ...
    if otel_initialized:
        try:
            from agents.servicenow_agent.src.utils.otel_tracing import shutdown_tracing
            shutdown_tracing()
    ...
    if langfuse_ok:
        try:
            from agents.servicenow_agent.src.orchestrator.llm_intelligence import langfuse_client as lf_client
            if lf_client:
                lf_client.flush()
...
def register_routes(app: FastAPI) -> None:
    ...
    async def get_incident(request: Request, incident_id: str):
        ...
        """
        from agents.servicenow_agent.src.infrastructure.postgres import get_postgres_pool
    ...
    async def close_incident(request: Request, incident_id: str, resolution: str = "Resolved"):
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                try:
                    publisher = request.app.state.publisher
                    from agents.servicenow_agent.src.streaming.event_publisher import IncidentEventType
    ...
    async def close_servicenow_incident(request: Request, incident_id: str):
        ...
        try:
            ...
            search_resp = hx.get(
                f"{snow_url}/api/now/table/incident",
                params={"sysparm_query": f"number={incident_id}", "sysparm_fields": "sys_id,number,state", "sysparm_limit": "1"},
                auth=(snow_user, snow_pass), timeout=15.0,
            )
            ...
            update_body = {"state": "6", "close_code": resolution_code, "close_notes": close_notes}
            if work_notes:
                update_body["work_notes"] = work_notes
            close_resp = hx.patch(f"{snow_url}/api/now/table/incident/{sys_id}", json=update_body, auth=(snow_user, snow_pass), timeout=15.0)
            if close_resp.status_code == 200:
                logger.info(f"ServiceNow incident {incident_id} closed via API")
                return {"status": "closed", "incident_id": incident_id, "sys_id": sys_id, "state": "6 - Resolved", "close_code": resolution_code}
            else:
                raise HTTPException(502, f"ServiceNow close failed: {close_resp.status_code}")
        except HTTPException:
            raise
        except Exception as e:
    ...
    async def submit_rag_feedback(request: Request):
        ...
        feedback_entry = {
            ...
            "workflow_steps_completed": body.get("workflow_steps_completed", 0),
            "timestamp": datetime.now().isoformat(),
        }

        try:
            feedback_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "data", "rag_feedback.json")
            ...
            logger.info(f"RAG feedback stored ({len(existing)} entries)")
        except Exception as e:
            logger.warning(f"Failed to persist RAG feedback: {e}")

        # Boost script keywords for helpful ratings
        if rating == "helpful" and script_used:
            for script in script_selector.scripts:
                if script.get("name") == script_used:
                    ...
                    logger.info(f"Boosted script '{script_used}' with {len(new_kw)} new keywords")
                    break

        return {
            "status": "success",
            "message": f"RAG knowledge base updated for {incident_id}",
            "incident_id": incident_id,
            "rating": rating,
    ...
    async def delete_all_incidents(request: Request):
        """Delete ALL incidents from ServiceNow and clear local caches."""
        import redis
        import httpx
        import json as json_lib
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{snow_url}/api/now/table/incident",
                    params={"sysparm_limit": 500, "sysparm_fields": "sys_id,number"},
                    auth=(snow_user, snow_pass),
                    headers={"Accept": "application/json"},
                    timeout=60.0,
                )
                if response.status_code != 200:
                    raise HTTPException(502, f"Failed to fetch incidents: HTTP {response.status_code}")

                incidents = response.json().get("result", [])
                logger.info(f"Found {len(incidents)} incidents to delete")
                ...
                for inc in incidents:
                    ...
                    try:
                        ...
                        if del_resp.status_code in (200, 204):
                            ...
                            failed_count += 1
                            errors.append(f"{number}: HTTP {del_resp.status_code}")
                    except Exception as e:
                        failed_count += 1
        ...
        try:
            ...
            redis_client.delete("incidents:active")
            cache_cleared = True
        except Exception as e:
        ...
        return {
            ...
            "message": f"Deleted {deleted_count} incidents from ServiceNow",
        }
    ...
    async def execute_langgraph_node(node_id: int, request: Request):
        ...
        try:
            from agents.servicenow_agent.src.orchestrator.metrics import (
                ...
                record_mcp_request, record_servicenow_request,
                record_script_match, SCRIPT_MATCHES,
                GITHUB_ACTIONS_RUNS,
            )
            _metrics_ok = True
        except Exception:
        ...
        try:
            from agents.servicenow_agent.src.orchestrator.llm_intelligence import langfuse_client as _lf, LANGFUSE_ENABLED as _lf_on
            _lf_trace = None
            if _lf_on and _lf:
        ...
        if not incident_description and incident_id:
            ...
            try:
                ...
                if snow_pass:
                    ...
                    if _metrics_ok:
                        record_servicenow_request("get_incident", "success" if resp.status_code == 200 else "error", time.time() - _snow_t)
        ...
        if node_id == 4:
            ...
            if _metrics_ok:
                record_mcp_request("rag", "script_match", "success" if matched_scripts else "no_match", time.time() - _rag_t)
                if matched_scripts:
        ...
        if node_id == 8:
            ...
            # Generate LLM reasoning for why this script was selected
            # Use RAG to find matching scripts and get match reasons
            matched_scripts = script_selector.match_scripts(incident_description, max_results=3)

            llm_reasoning = f"Based on incident analysis: '{incident_description}'\n\n"
            if matched_scripts and script_info:
                top_match = matched_scripts[0]
                match_reasons = top_match.get("match_reasons", [])

                llm_reasoning += f"Selected '{script_name}' because:\n"
                if match_reasons:
                    for i, reason in enumerate(match_reasons[:3], 1):
                        llm_reasoning += f"{i}. {reason}\n"
                else:
                    llm_reasoning += f"• Best match with {top_match.get('score', 0):.1%} confidence\n"
                    llm_reasoning += f"• Script purpose: {script_description}\n"

                llm_reasoning += f"\nRisk assessment: {risk_level.upper()}"
                llm_reasoning += f"\nEstimated execution time: ~2-3 minutes"
            else:
                llm_reasoning += f"This script matches the incident type and severity.\n"
                llm_reasoning += f"Script purpose: {script_description}\n"
                llm_reasoning += f"Risk level: {risk_level}"
        ...
        if node_id == 9:
            ...
            # Record GitHub Actions MCP metrics
            if _metrics_ok:
                _gh_status = "success" if github_status == "triggered" else "error"
                record_mcp_request("github", "trigger_workflow", _gh_status, time.time() - node_start_time)
                GITHUB_ACTIONS_RUNS.labels(workflow="shell-execute.yml", status=_gh_status).inc()
        ...
        if node_id == 11:
            ...
            if snow_pass and incident_id:
                try:
                    ...
                    if search_resp.status_code == 200:
                        ...
                        if records:
                            sys_id = records[0]["sys_id"]
                            close_notes = f"Automated resolution via AI Agent Platform. Script executed: {script_name}. Workflow completed."
                            work_notes = f"[AI Agent] Incident resolved automatically.\nScript: {script_name}\nWorkflow ID: {workflow_id}"
                            close_resp = hx.patch(
                                f"{snow_url}/api/now/table/incident/{sys_id}",
                                json={"state": "6", "close_code": "Solved (Permanently)", "close_notes": close_notes, "work_notes": work_notes},
                                headers=snow_headers,
                                auth=(snow_user, snow_pass),
                                timeout=15.0,
                            )
                            if close_resp.status_code == 200:
                                ...
                                # ACL restriction — try work_notes only
                                wn_resp = hx.patch(
                                    f"{snow_url}/api/now/table/incident/{sys_id}",
                                ...
                                if wn_resp.status_code == 200:
                                    ...
                                    close_details = {"sys_id": sys_id, "message": "Work notes added (state change restricted by ACL)"}
                                else:
                                    close_status = "simulated"
                                    close_details = {"message": "ServiceNow ACL restricts updates. Resolution recorded locally."}
                            else:
                                close_status = f"error: ServiceNow returned {close_resp.status_code}"
                                logger.warning(f"Failed to close {incident_id}: {close_resp.status_code}")
                        else:
            ...
            # Record ServiceNow close MCP metrics
            if _metrics_ok:
                _snow_status = "success" if close_status in ("closed", "work_notes_added") else "error"
                record_servicenow_request("close_incident", _snow_status, time.time() - node_start_time)
                record_mcp_request("servicenow", "close_ticket", _snow_status, time.time() - node_start_time)
    ...
    def _jira_request(method: str, path: str, json_body: dict = None, params: dict = None) -> dict:
        ...
        if not all([jira_url, jira_user, jira_token]):
            raise HTTPException(400, "Jira not configured. Set JIRA_URL, JIRA_USERNAME, JIRA_API_TOKEN.")

        url = f"{jira_url}/rest/api/3{path}"
        auth = _HTTPBasicAuth(jira_user, jira_token)
        headers = {"Accept": "application/json", "Content-Type": "application/json"}

        resp = req.request(method, url, auth=auth, headers=headers, json=json_body, params=params, timeout=30)
    ...
    def _parse_jira_issue(issue: dict) -> dict:
        fields = issue.get("fields", {})
        raw_desc = fields.get("description")
        if isinstance(raw_desc, dict):
            try:
                desc_text = raw_desc.get("content", [{}])[0].get("content", [{}])[0].get("text", "")
            except (IndexError, AttributeError):
                desc_text = ""
        else:
            desc_text = raw_desc or ""
        return {
            "ticket_id": issue["key"],
    ...
    async def get_jira_tickets(request: Request):
        ...
        try:
            data = _jira_request("GET", "/search/jql", params={"jql": jql, "maxResults": max_results, "fields": "summary,status,priority,issuetype,assignee,reporter,labels,created,updated,description"})
        except HTTPException:
            raise
        except Exception as e:
            return {"tickets": [], "count": 0, "source": "jira", "error": str(e)}
        tickets = [_parse_jira_issue(issue) for issue in data.get("issues", [])]
    ...
    async def create_jira_ticket(request: Request):
        """Create a new Jira ticket. Body: { summary, description, issue_type, priority, labels, project_key }"""
        body = await request.json()
        ...
        issue_payload = {
            "fields": {
                ...
                "summary": summary,
                "description": {"version": 1, "type": "doc", "content": [{"type": "paragraph", "content": [{"type": "text", "text": description_text}]}]},
        ...
        return {"success": True, "ticket_id": ticket_key, "ticket_url": f"{jira_url}/browse/{ticket_key}"}

    @app.post("/api/jira/push-incident")
    async def push_incident_to_jira(request: Request):
        """Push an incident to Jira. Body: { incident_id, short_description, description, priority, category, labels }"""
        body = await request.json()
        incident_id = body.get("incident_id", "")
        ...
        return {"success": True, "incident_id": incident_id, "jira_ticket_id": ticket_key, "jira_ticket_url": f"{jira_url}/browse/{ticket_key}"}
    ...
    async def jira_webhook(request: Request):
        ...
        if issue_type in ["Story", "Task"] and "data" in issue.get("fields", {}).get("summary", "").lower():
            from agents.servicenow_agent.src.streaming.event_publisher import PipelineEventType
    ...
    async def get_dataproc_cluster_status(request: Request):
        ...
        try:
            from agents.servicenow_agent.src.infrastructure import get_dataproc_client
    ...
    async def create_data_pipeline(request: Request):
        ...
        """
        from agents.servicenow_agent.src.control_plane.handlers.data_agent_handler import PipelineIntent
        import uuid
    ...
    async def rag_search(request: Request):
        ...
        try:
            from agents.servicenow_agent.src.rag.intelligent_retriever import intelligent_retriever
    ...
    async def rag_stats(request: Request):
        ...
        try:
            from agents.servicenow_agent.src.rag.intelligent_retriever import intelligent_retriever
            from agents.servicenow_agent.src.rag.hybrid_search_engine import HybridSearchEngine
    ...
    # ==========================================================================
    # Chat API (used by FloatingChat and IncidentChat components)
    # ==========================================================================

    @app.post("/api/v1/chat")
    async def chat_endpoint(request: Request):
        """
        Chat endpoint for incident-aware AI assistant.
        ...
        Returns:
            {
                "response": "Based on similar incidents...",
                "confidence": 0.85,
                "sources": ["RAG", "Graph DB"],
                "reasoning": "Found 3 similar incidents...",
                "similar_incidents": [...]
            }
        """
        body = await request.json()
        ...
        try:
            # Search RAG for similar incidents/scripts
            similar_results = []
            rag_confidence = 0.0
            ...
            try:
                ...
                if incident_id:
                    ...
                    desc = incident_data.get("short_description", "") or incident_data.get("description", "")
                    if desc:
                        query = f"{desc} {message}"

                results = retriever.retrieve(query=query, top_k=3)
                similar_results = [
                    {
                        "id": r.script_id,
                        "content": f"{r.name}: {r.description[:200] if hasattr(r, 'description') and r.description else ''}",
                        "score": r.final_score,
                        "metadata": {"script_type": r.script_type, "risk_level": r.risk_level},
                    }
                    for r in results
                ]
                if similar_results:
                    rag_confidence = similar_results[0].get("score", 0)
            except Exception as rag_err:
                logger.warning(f"RAG search failed (non-blocking): {rag_err}")

            # Also try local script matching
            try:
                script_selector = getattr(app.state, 'script_selector', None)
                if script_selector is None:
                    script_selector = ScriptSelector()
                    app.state.script_selector = script_selector
                matched_scripts = script_selector.match_scripts(message, max_results=3)
            except Exception:
                matched_scripts = []

            # Build response from RAG results + script matches
            response_parts = []

            if similar_results:
                response_parts.append(f"I found {len(similar_results)} relevant results from the knowledge base:")
                for i, r in enumerate(similar_results, 1):
                    content = r.get("content", "")[:200]
                    score = r.get("score", 0)
                    response_parts.append(f"\n{i}. (Score: {score:.0%}) {content}")

            if matched_scripts:
                response_parts.append(f"\n\nMatched {len(matched_scripts)} remediation scripts:")
                for i, s in enumerate(matched_scripts, 1):
                    name = s.get("name", s.get("script_name", "Unknown"))
                    score = s.get("score", 0)
                    response_parts.append(f"\n{i}. {name} (Match: {score:.0%})")

            if not response_parts:
                response_parts.append(
                    f"I'm analyzing incident {incident_id}. "
                    "I couldn't find closely matching past incidents in the knowledge base. "
                    "Please provide more details about the symptoms or error messages."
                )

            confidence = max(rag_confidence, matched_scripts[0].get("score", 0) if matched_scripts else 0)

            return {
                "response": "\n".join(response_parts),
                "confidence": round(confidence, 2),
                "sources": ["RAG", "Script Registry"] if similar_results or matched_scripts else ["Local"],
                "reasoning": f"Searched RAG ({len(similar_results)} results) and script registry ({len(matched_scripts)} matches)",
                "similar_incidents": similar_results[:3],
            }
