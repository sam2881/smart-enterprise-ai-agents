...
class NLTransformProcessor:
    ...
    def _generate_rule_based(self, request: TransformRequest, intent: TransformIntent) -> Optional[GeneratedCode]:
        ...
        elif intent == TransformIntent.JOIN:
            # Extract join components from NL
            join_config = self._extract_join_config(description, schema)
            if join_config:
                code = self.generate_join_code(
                    left_alias="df",
                    right_table=join_config["right_table"],
                    join_keys=join_config["join_keys"],
                    join_type=join_config["join_type"],
                    select_columns=join_config.get("select_columns"),
                )
                return GeneratedCode(
                    pyspark_code=code,
                    intent=intent,
                    confidence=0.75,
                    explanation=(
                        f"{join_config['join_type'].upper()} JOIN with "
                        f"{join_config['right_table']} on "
                        f"{', '.join(k['left'] + '=' + k['right'] for k in join_config['join_keys'])}"
                    ),
    ...
    def _extract_join_config(
        self, description: str, schema: Dict[str, str]
    ) -> Optional[Dict[str, Any]]:
        """
        Extract structured JoinConfig from natural language.

        Handles patterns like:
          - "join orders with customers on customer_id"
          - "left join products on orders.product_id = products.id"
          - "join customer_master using customer_id, keep all orders"
          - "combine sales with regions on region_code"
        """
        # Detect join type from NL
        join_type = "inner"
        desc_lower = description.lower()
        if any(kw in desc_lower for kw in ["left join", "keep all", "all rows from"]):
            join_type = "left"
        elif any(kw in desc_lower for kw in ["right join"]):
            join_type = "right"
        elif any(kw in desc_lower for kw in ["full join", "full outer", "all from both"]):
            join_type = "full"
        elif any(kw in desc_lower for kw in ["cross join", "cartesian"]):
            join_type = "cross"

        # Extract table name (the thing being joined to)
        table_patterns = [
            r"(?:join|combine|merge)\s+(?:with\s+)?(\w+)",
            r"(?:join|combine|merge)\s+(\w+)\s+(?:with|to|on|using)",
            r"(?:with|to)\s+(\w+)\s+(?:on|using|where)",
        ]
        right_table = None
        for pat in table_patterns:
            ...
            if match:
                ...
                if candidate not in ("the", "a", "an", "table", "data", "all"):
        ...
        # Extract join keys
        join_keys = []
        # Pattern: "on col1 = col2" or "on col1"
        key_match = re.search(
            r"(?:on|using)\s+(\w+)(?:\s*=\s*(\w+))?", desc_lower
        )
        if key_match:
            left_key = key_match.group(1)
            right_key = key_match.group(2) or left_key
            join_keys.append({"left": left_key, "right": right_key})
        else:
            # Try to find common column names between description and schema
            words = re.findall(r'\b\w+_?(?:id|key|code)\b', desc_lower)
            for word in words:
                if word in schema or word in {k.lower() for k in schema}:
                    join_keys.append({"left": word, "right": word})
                    break

        if not join_keys and join_type != "cross":
            return None
