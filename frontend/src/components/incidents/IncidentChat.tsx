              ...
              >
                ...
                {message.metadata && message.role === 'assistant' && (
                  <div className="mt-3 pt-3 border-t border-purple-200 space-y-2 text-sm">
                    ...
                    {message.metadata.similar_incidents && message.metadata.similar_incidents.length > 0 && (
                      <div className="mt-2">
                        <span className="text-purple-600 font-medium text-xs">Top Similar Incidents:</span>
                        <div className="mt-1 space-y-1">
                          {message.metadata.similar_incidents.slice(0, 3).map((si, idx) => (
                            <div key={idx} className="bg-white rounded p-2 border border-purple-100 text-xs">
                              <div className="flex justify-between items-center">
                                <span className="font-medium text-gray-800">{si.id || `Result ${idx + 1}`}</span>
                                <span className="text-purple-600">{(si.score * 100).toFixed(0)}% match</span>
                              </div>
                              <p className="text-gray-600 mt-1 line-clamp-2">{si.content?.slice(0, 120)}...</p>
                            </div>
                          ))}
                        </div>
                      </div>
