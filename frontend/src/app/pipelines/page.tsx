...
/**
 * Pipelines Page - Data Agent UI (Jira-Centric)
 *
 * Enterprise pipeline management with Jira integration:
 * - All pipelines are scoped to Jira tickets (parent-child relationship)
 * - View active/completed pipelines with Jira context
 * - Monitor pipeline generation progress in real-time
 * - Approve/reject pipeline deployments
 * - Create pipelines only through Jira ticket context
 */
...
type ViewMode = 'list' | 'create' | 'progress' | 'jira-select'
...
// Extended pipeline item with Jira context
interface ExtendedPipelineListItem extends PipelineListItem {
  jira_ticket?: string
}
...
function PipelineRow({ pipeline, onViewProgress }: { pipeline: ExtendedPipelineListItem; onViewProgress: (id: string) => void }) {
  const isInProgress = ['planning', 'generating', 'validating', 'deploying'].includes(pipeline.status)
  const jiraTicket = pipeline.jira_ticket || (pipeline as any).jira_ticket
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3 mb-1">
          ...
          {jiraTicket && (
            <Link
              href={`/jira/${jiraTicket}`}
              onClick={(e) => e.stopPropagation()}
              className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-purple-500/20 text-purple-400 border border-purple-500/30 hover:bg-purple-500/30 transition-colors"
            >
...
function PipelinesPageContent() {
  ...
  const createPipeline = useMutation({
    mutationFn: async (input: UnifiedPipelineInput): Promise<CreatePipelineResponse> => {
      return await api.createPipelineUnified(input)
    },
    onSuccess: (data) => {
  ...
  const filteredPipelines = useMemo(() => {
    const pipelines = (pipelinesData?.pipelines || []) as ExtendedPipelineListItem[]

    return pipelines.filter((pipeline) => {
      if (searchQuery) {
        const query = searchQuery.toLowerCase()
        const jiraTicket = pipeline.jira_ticket || ''
        const matchesSearch =
          ...
          jiraTicket.toLowerCase().includes(query)
      ...
      if (jiraFilter) {
        const jiraTicket = pipeline.jira_ticket || ''
        if (!jiraTicket.toLowerCase().includes(jiraFilter.toLowerCase())) {
          return false
        }
      }
  ...
  }, [pipelinesData, searchQuery, statusFilter, envFilter, jiraFilter])

  const handleSubmit = async (input: UnifiedPipelineInput) => {
  ...
  const hasActiveFilters = searchQuery || statusFilter || envFilter || jiraFilter
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 max-w-md w-full mx-4 shadow-2xl">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white">Jira Ticket Required</h3>
                  <p className="text-sm text-gray-400">Pipelines must be created within Jira context</p>
                </div>
              </div>

              <div className="p-4 bg-gray-900/50 rounded-lg border border-gray-700 mb-6">
                <p className="text-sm text-gray-300 mb-3">
                  All data pipelines are linked to Jira tickets for:
                </p>
        ...
        <div className="bg-gray-800/30 rounded-xl border border-gray-700/30 p-4 mb-6">
          <div className="flex flex-wrap items-center gap-4">
            ...
            {/* Jira Ticket Filter */}
            <div className="relative">
              <Ticket className="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" />
              <input
                type="text"
                placeholder="Jira ticket..."
                value={jiraFilter}
                onChange={(e) => setJiraFilter(e.target.value)}
                className="bg-gray-800 border border-gray-700 text-gray-300 text-sm rounded-lg pl-8 pr-3 py-2 w-32 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-colors placeholder-gray-500"
              />
            </div>
          ...
          <div className="text-center py-20 bg-gray-800/30 rounded-xl border border-gray-700/30">
            ...
            <p className="text-gray-400 mb-6">
              {hasActiveFilters
                ...
                : 'Create your first data pipeline from a Jira ticket.'}
            </p>
            {hasActiveFilters ? (
