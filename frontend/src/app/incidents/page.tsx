...
export default function IncidentsPage() {
  ...
  const [deleteResult, setDeleteResult] = useState<{ deleted: number; message: string } | null>(null)

  // Delete all incidents from ServiceNow + cache
  const deleteAllIncidents = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/incidents`, { method: 'DELETE' })
      ...
      setDeleteResult({ deleted: data.deleted, message: data.message })
      // Refresh the list
      ...
      <div className="min-h-screen p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={isDeleting || incidents.length === 0}
              className="flex items-center gap-2 px-4 py-2.5 text-sm text-red-400 hover:text-white bg-red-500/10 hover:bg-red-500/20 rounded-xl border border-red-500/30 hover:border-red-500/50 transition-all disabled:opacity-50"
            >
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="bg-[#161b22] border border-red-500/30 rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
              <div className="flex items-center gap-3 mb-4">
                <div className="p-3 rounded-xl bg-red-500/20">
                  <Trash2 className="w-6 h-6 text-red-400" />
                </div>
                <h3 className="text-xl font-bold text-white">Delete All Incidents</h3>
              </div>
              <p className="text-gray-400 mb-6">
                This will <span className="text-red-400 font-semibold">permanently delete all {incidents.length} incidents</span> from
                ServiceNow and clear the local cache. This action cannot be undone.
              </p>
              <div className="flex items-center gap-3 justify-end">
                <button
                  onClick={() => setShowDeleteConfirm(false)}
                  className="px-5 py-2.5 text-sm text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={deleteAllIncidents}
                  className="px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 rounded-xl shadow-lg shadow-red-500/25 transition-all"
                >
                  Yes, Delete All
                </button>
              </div>
        ...
        {deleteResult && (
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-4 mb-8 flex items-center gap-4">
            <div className="p-2 rounded-lg bg-emerald-500/20">
              <CheckCircle className="w-5 h-5 text-emerald-400" />
            </div>
            <span className="text-emerald-400 flex-1">{deleteResult.message}</span>
            <button
