...
def process_silver_to_gold(
    ...
    """
    # Step 0: Resolve join dependencies (multi-table joins)
    if join_configs:
        from spark_jobs.join_executor import execute_joins
        print(f"Resolving {len(join_configs)} join dependencies...")
        base_df = spark.table(contract_config["silver_table"])
        base_df = base_df.filter(col("_silver_execution_id") == execution_id)
        gold_df = execute_joins(
            spark=spark,
            base_df=base_df,
    ...
    elif gold_view and gold_view.get("view_sql"):
    ...
    # Step 4.5: Apply PII masking for classified columns before Gold output
    try:
        from security.governance_enforcer import GovernanceEnforcer
        enforcer = GovernanceEnforcer()
        asset_name = contract_config.get("gold_table") or contract_config.get("gold_path", "")
        # Also check Silver asset classifications (masking carries forward)
        silver_asset = contract_config.get("silver_table") or contract_config.get("silver_path", "")
        gold_df = enforcer.enforce_pii_masking(gold_df, silver_asset)
        print("PII masking enforced for Gold output")
    except ImportError:
        pass  # Governance module not available
    except Exception as e:
        print(f"Warning: PII masking for Gold failed (non-blocking): {e}")
...
def main():
    ...
    try:
        ...
        # Load join dependencies (multi-table joins)
        try:
            from spark_jobs.join_executor import load_join_dependencies
            join_configs = load_join_dependencies(spark, args.metadata_db, args.contract_id)
        except Exception:
            join_configs = []
        ...
        metrics = process_silver_to_gold(
            ...
            join_configs=join_configs if join_configs else None,
        )
