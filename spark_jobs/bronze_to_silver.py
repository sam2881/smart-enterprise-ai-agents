...
def process_bronze_to_silver(
    ...
    # Step 2.5: PII detection + masking enforcement
    try:
        from security.pii_detection import detect_pii, persist_classifications
        from security.governance_enforcer import GovernanceEnforcer

        pii_results = detect_pii(silver_df)
        if pii_results:
            # Persist classifications to data_classification table
            asset_name = contract_config.get("silver_table") or contract_config.get("silver_path", "")
            persist_classifications(pii_results, asset_name)

            # Apply masking enforcement
            enforcer = GovernanceEnforcer()
            silver_df = enforcer.enforce_pii_masking(silver_df, asset_name)
            print(f"PII masking applied for {len(pii_results)} classified columns")
    except ImportError:
        pass  # PII detection modules not available
    except Exception as e:
        print(f"Warning: PII detection/masking failed (non-blocking): {e}")
