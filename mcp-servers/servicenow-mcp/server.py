...
class ServiceNowClient:
    ...
    def __init__(self, config: ServiceNowConfig):
        self.config = config
        # Extract only instance name (e.g., dev349960 from https://dev349960.service-now.com)
        # pysnow will automatically append .service-now.com
        instance = config.instance_url.replace("https://", "").replace("http://", "")
        instance = instance.split(".")[0]  # Get only the subdomain part
    ...
    def fetch_recent_incidents(self, since_minutes: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            # Use QueryBuilder for proper datetime comparison
            from pysnow import QueryBuilder
            query = (QueryBuilder()
                    .field('active').equals('true')
                    .AND()
                    .field('sys_updated_on').greater_than_or_equal(since_time))
